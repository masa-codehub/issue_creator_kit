...
- **Action:** 
    ...
    2. **Todo作成 (`save_memory`):** 目的達成に必要な手順をTodoリストとして作成し、`save_memory` ツールを使用して自身の記憶に保存・提示する。
        - **ステータス管理テンプレート:** 
            ...
            - `[-]`: スキップ (Skipped)
        - **ツールの使用例:** 
          `save_memory{fact: "Current Todo: [x] 実装タスク1 (成果物: src/utils.py), [ ] テストタスク2 (成果物: tests/test_utils.py)"}`

    3. **セルフレビューと修正 (Self-Correction Loop):** 
       作成したTodoリストを以下の観点で厳しくレビューし、問題がなくなるまで修正を繰り返す。
        - **セルフレビュー観点:**
            - **成果物は具体的か？** （単に「実装する」ではなく、「`src/utils.py`を作成する」とファイルパスまで特定できているか）
            - **手順に飛躍はないか？** （前提となるファイルが存在しないのに読み込もうとしていないか）
            - **リスクへの対策は含まれているか？** （テスト失敗時の予備動作など）
        - **Action:**
            - 問題があれば、即座にTodoを修正して `save_memory` を更新する。
            - 修正後、再度セルフレビューを行う。
            - 全ての懸念が解消されるまで繰り返し、僅かな懸念も消えたら次のStateに移行する。

### State 2: Execution (実行)
- **いつ:** Todoリストがあり、Todoの内容に僅かな懸念も消えたとき
- **Action:** 
    1. **実行と適応:** Todoを順次実行する。
