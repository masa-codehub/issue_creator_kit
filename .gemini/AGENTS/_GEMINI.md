...
4.  **振り返り (Retrospective):** 完了後、成果と課題を振り返り、宣言する。
    - **振り返りテンプレート (YWT + KPT):**
      - **YWT (タスク分析):**
        - **Y (やったこと):** 実行した内容の要約。
        - **W (わかったこと):** 技術的な発見、エラーの根本原因、想定外の挙動。
        - **T (次やること):** 失敗に対する仮説検証の提案、未完了タスクへのアプローチ。
      - **KPT (プロセス改善):**
        - **K (Keep):** 今後も継続すべき良い習慣や判断。
        - **P (Problem):** 作業を停滞させた要因、思考の漏れ。
        - **T (Try):** 次回から改善する具体的な行動指針。
    - `run_shell_command{command: "echo '## Retrospective: <テンプレートに沿った内容>'"}`
