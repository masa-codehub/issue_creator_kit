...
### 基本的なワークフロー
タスクを確実に遂行するため、以下のサイクルを繰り返す。

1.  **目標設定 (Goal Setting):** ユーザーとの会話から具体的な目標を特定し、**SMARTの法則**に基づいて宣言する。
    - **SMARTの法則:**
      - **Specific (具体性):** 何を達成するか明確にする。
      - **Measurable (計量性):** 完了したことをどう確認するか（テストのパス、PRの作成など）。
      - **Achievable (達成可能性):** 現状のツールと制約で実行可能か。
      - **Relevant (関連性):** ユーザーの最終的な意図に沿っているか。
      - **Time-bound (期限):** このサイクル内でどこまで終わらせるか。
    - **宣言の例:**
      `run_shell_command{command: "echo '## Goal: <目標の内容> (SMARTに基づいた記述)'"}`
2.  **Todo作成 (`save_memory`):** 目的達成に必要な手順をTodoリストとして作成し、`save_memory` ツールを使用して自身の記憶に保存した上で、ユーザーに提示する。
    - **ステータス管理テンプレート:**
      - `[ ]`: 未実行 (Pending)
      - `[x]`: 完了 (Completed)
...
4.  **振り返り (Retrospective):** 完了後、成果と課題を振り返り、宣言する。
    - 振り返り完了後、`save_memory` に保存したTodoリストの内容を削除（または完了済みとしてクリア）し、次回のタスクに備える。
    - `run_shell_command{command: "echo '## Retrospective: <振り返りの内容>'"}`
5.  **次のサイクルへ:** 必要に応じて1に戻り、新たな目標を設定する。

### 外部への報告

- **Issueへの報告:** 進捗や結果を報告する際は `add_issue_comment` を使用する。
