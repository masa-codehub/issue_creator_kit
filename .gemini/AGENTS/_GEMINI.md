...
- プルリクエストの管理 (PR Protocol)
  ...
  3. **新規作成 (Creation):**
     ...
     PR作成前に必ず思考プロセス内で以下のテンプレートを埋め、**独自の要約や省略をせず**に `body` パラメータへ指定すること。
...
## 1. SSOT精査と整合性検証プロトコル (Conceptual Integrity & Scrutiny)
設計や計画の妥当性を保証するため、既読・未読にかかわらず、以下の手順で「立ち止まって再検証」することを標準化します。

1.  **重要ドキュメントの再読 (Fresh Read):** 
    - `docs/system-context.md` や関連する承認済み ADR (`reqs/design/_approved/`) を、**新規に `read_file` で読み込む**。以前のターンで読み込んでいても、キャッシュに頼らず最新の状態を確認することを優先する。
2.  **ドメイン観点での精査 (Scrutiny):**
    - 読み込んだドキュメントと、現在の提案内容を以下の観点で突き合わせる。
        - **用語の一貫性:** ユビキタス言語から逸脱していないか？
        - **境界の遵守:** システム境界（In-Scope/Out-of-Scope）を侵していないか？
        - **原則の継承:** 過去の ADR で決定した設計原則や戦略的トレードオフに反していないか？
3.  **考察結果の出力 (Synthesis):**
    - 単に「矛盾なし」とするのではなく、**「XXドキュメントのYYという記述に基づき、今回のZZという設計は適切である。なぜなら...」** という論理的な考察（思考プロセス）を詳細に出力する。

## 2. Pythonにおける静的解析とテスト
