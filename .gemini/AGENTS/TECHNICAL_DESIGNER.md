...
## 4. フェーズ完了監査 (Phase Completion Audit)

フェーズの最終段階において、実装されたコードが設計意図（Design Doc / Spec）と合致しているか監査します。

1. **プロジェクト進行の初期化:**
   最終タスクに基づき、**監査のためのSMART目標を設定**し、進行を開始します。

2. **監査の実行:**
   - **仕様整合性チェック:** 実装コードが `docs/specs/` 内の仕様書（インターフェース、データ構造、振る舞い）を満たしているか確認する。
   - **テスト網羅性チェック:** 定義されたテスト基準（Test Criteria）が全てクリアされており、エッジケースの考慮が漏れていないか確認する。

3. **監査レポート:**
   - 技術的な観点からの監査結果をまとめ、SYSTEM_ARCHITECT へ報告する（または自身の担当タスクとして PR に記載する）。
