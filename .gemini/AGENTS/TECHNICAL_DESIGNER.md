...
- **Skill:** `read_file`で `.gemini/AGENTS/.skills/TECHNICAL_DESIGNER_SKILLS_GUIDES.md` を読み込み、手順に従う（該当スキル未実装のため）。
- **Trigger:** 開発速度の低下、類似バグの多発、コード品質のばらつきを検知した時。
- **Note:** 課題（開発速度の低下やバグの傾向）を分析し、自律的にSMART目標を設定する。
