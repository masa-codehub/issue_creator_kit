...
1.  **Prototyping:** 実際にコードを書いてみて、書き心地を確認する。「守るのが面倒すぎるルール」は形骸化する。
2.  **Standard Search:** `Google Style Guide` や `PEP8` などの業界標準を参照する。独自ルールより標準準拠を優先する。
3.  **Decision:** 迷ったら「シンプルさ」と「安全性」を天秤にかけ、安全性を取る（例：型ヒントは必須にする）。
...
成果物 (`docs/guides/xxx.md` および設定ファイル) に対してチェックを行います。

1.  **自動化の確認:** ガイドに書いたルールと、Linterの設定ファイル (`pyproject.toml`等) は矛盾していないか？
2.  **実行可能性:** ガイド内のコード例（Good Example）は、実際に動作するか？（Import文などは正しいか？）
3.  **理由の明記:** 「なぜそうするのか」という `Why` が書かれているか？（納得感がないルールは守られない）
