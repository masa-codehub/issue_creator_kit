...
作成した実装計画（ドラフト）に基づき、スピードと安全性のトレードオフや、リソース配分についてユーザーと合意形成を行います。全ての論点が解消されるまで、以下の **「合意形成ループ」** を回し続けます。

1.  **論点（質問リスト）の初期化:**
    - Step 2 で作成した WBS や特定されたリスクに基づき、合意が必要な論点を特定する。
    - `save_memory` を用いて、解決すべき項目を記録する。
    - **主な論点例:**
        - **並行稼働期間:** 安全重視で長期間とるか、コスト重視で早期に切り替えるか。
        - **ロールバック基準:** どのようなエラーが発生したら即時切り戻しを行うか。
        - **データ移行:** サービス停止（メンテ）を許容するか、完全無停止（Dual Write）で行うか。
    - **コマンド例:** `save_memory(fact="[Roadmap-XXX] 論点TODO: 1.並行稼働(Shadowing)の期間設定(1週間か1ヶ月か)...")`

2.  **合意形成ループの実行:**
    - **A. 最小単位の問いかけ:**
        - **TODO の呼び出し:** `save_memory` に記録した論点から優先度の高い項目を選択する。
        - **トレードオフの提示:** 「A案（安全だが遅い）とB案（早いがリスクあり）」のように選択肢とそれぞれのメリット・デメリットを提示し、ユーザーの判断を仰ぐ。
    - **B. 計画の同期と提示:**
        - ユーザーの回答に基づき、`reqs/_inbox/roadmap-XXX-implementation.md` のスケジュールや WBS を `replace` で即座に更新する。
        - **更新の明示:** どのタスクが追加・削除されたか、期間がどう変更されたかを要約して提示する。
    - **C. リストの更新:** 解決した論点を削除し、新たな疑問があれば `save_memory` に追加する。
    - **D. 実現可能性の監視:**
        - ユーザーの要望が、技術的な制約やリソース（期間・予算）と矛盾していないか常に監視する。無理がある場合は代替案を提示して再調整する。

3.  **ループの終了（完了判定）:**
    - `save_memory` 内の全ての論点が解消され、最新のロードマップの内容および **「検証基準 (Verification Criteria)」** に対してユーザーと合意・確信を共有できた時点で、このフェーズを完了とする。
