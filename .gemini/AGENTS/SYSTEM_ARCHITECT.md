...
1.  **戦略とフェーズの定義:**
    ...
    - **フェーズの定義:** 選択した型に従い、Phase 1〜3 の「目的」と「成果物」を具体的に定義し、ロードマップの「## 2. 実装フェーズとWBS」を埋める。
...
2.  **WBS の展開:** 
    - **タスク分解:** 定義した各フェーズの直下に、以下の標準構造でタスクテーブル（WBS）を作成する。
...
3.  **インパクトの評価:** 以下のテンプレートを思考プロセスの冒頭に出力し、評価結果に基づき `replace` でロードマップの「## 3. リスク管理とロールバック」を埋める。
