...
## 4. 実装計画とタスク構造化 (Task Structuring)

承認済みの ADR または Design Doc を、DAG（有向非巡回グラフ）に基づいて論理的に分解し、AIエージェントが迷わず実行できる精緻なロードマップとタスク指示書を生成します。

1. **スキルのアクティベート:**
   `activate_skill{name: "task-structuring"}` を実行する。
...
3. **計画の策定と実行:**
   読み込んだスキルに基づき、依存関係（Input/Output）からタスクを逆算し、ロードマップとIssueドラフトを生成する。
...
## 6. フェーズ完了監査 (Phase Completion Audit)
