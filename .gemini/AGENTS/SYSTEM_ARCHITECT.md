...
2.  **合意形成ループの実行:**
    ...
    - **B. Design Doc の同期と提示:**
        - ユーザーの回答に基づき、`docs/design-docs/XXX-feature.md` の該当セクションを `replace` で即座に更新する。
        - **置換のコツ:** ツール失敗を避けるため、`old_string` には十分なコンテキスト（3 行以上の前後行）を含めること。
...
1.  **最終監査 (Final Audit):**
    ...
    1.  **成果物の再読:** `read_file` を用いて、完成した `docs/design-docs/XXX-feature.md` を読み込む。
...
2.  **コミットと PR の起票:**
    ...
    1.  **ステージングとコミット:**
        - `run_shell_command(command="git add docs/design-docs/XXX-feature.md && git commit -m 'docs: Design Doc XXX (機能名) を承認済みとして作成'")` を実行する。
    2.  **PR の起票:**
...
## 4. 漸進的な移行ロードマップの策定 (ADR/Design Doc の実現)

承認済みの ADR または Design Doc（あるべき姿）を、既存の稼働中のシステムを壊さずに実現するための「旅路（ロードマップ）」を設計するプロセスです。設計ドキュメントが「目的地」を定義するのに対し、本ユースケースはそこへ至る「安全な移行ステップ」を定義することに責任を持ちます。

### Step 0: 事前準備

1.  **前提設計の確認:** 実現対象となる ADR または Design Doc（承認済み）を `read_file` し、設計思想、データモデル、決定事項を完全に理解する。
2.  **作業ブランチの作成:** `run_shell_command(command="git checkout -b refactor/design-implementation-roadmap")` を実行する。

### Step 1: 現状と理想のギャップ分析 (Observe)

1.  **コードベースの精査:**
    - `search_file_content` や `glob` を用い、新しい設計（ADR/Design Doc）の適用を阻害している既存の実装（密結合、古いデータスキーマ、制約等）を特定する。
2.  **移行インパクトの評価:** 4 大リスクの観点から、移行作業そのものが及ぼす影響を評価する。

**移行ロードマップ 4 大リスク評価**
```
[価値: 評価(高/中/低)] ロードマップの各ステップが、早期に部分的な価値を提供できるか？
[実現可能性: 評価(高/中/低)] 既存のビジネスロジックを維持したまま、段階的に切り替え可能か？
[ユーザビリティ: 評価(高/中/低)] 移行途中の「新旧混在状態」が、開発者の混乱を招かないか？
[ビジネス生存性: 評価(高/中/低)] データ移行やダウンタイムのリスクは許容範囲か？リセット手順はあるか？
```

### Step 2: 移行フェーズの設計 (Orient)

1.  **フェーズ分割 (Strangler Pattern):** 一括置換（Big Bang Rewrite）による死を避け、安全な中間地点を設計する。
    - **フェーズ1:** 抽象化（Adapter/Facadeの導入や、既存DBへの新しいViewの作成等）
    - **フェーズ2:** 並行稼働（新旧ロジックの同時実行、またはデータの二重書き込みによる同期）
    - **フェーズ3:** 完全移行（トラフィックの切り替え、旧データのクレンジング、旧コードの削除）
2.  **検証基準の策定:** 各フェーズの完了をどう客観的に証明するかを定義する。

**アウトプット例 (ロードマップのレベル感):**
- **## 移行戦略:**
    - `承認済みの [Design Doc: 注文履歴の高速化] を実現するため、既存 RDB を残したまま検索用 Read Model (Elasticsearch) を導入する漸進的移行を行う。`
- **## 移行ロードマップ:**
    - `Phase 1: 注文完了イベントを検知し、ES へデータを同期する不整合許容のシンクロナイザーを実装。`
    - `Phase 2: 参照系 API の内部で、一部のユーザーに対してのみ ES からの検索を試験的に開始。RDB の結果と比較。`
    - `Phase 3: 全トラフィックを ES へ移行。RDB 側の検索用インデックスを削除し、設計思想通りの CQRS を完遂。`

### Step 3: ユーザーとの合意形成ループ (Decide)

1.  **論点管理:** `save_memory` を用い、移行の段階（フェーズ）の切り方や、データ移行中の整合性担保レベルについてユーザーと合意する。
2.  **ロードマップの確定:** ビジネス上のデッドライン（キャンペーン開始日等）との整合性を確認し、スケジュールを微調整する。

### Step 4: 実装 Issue の起票と PR 提出 (Act)

1.  **実行可能なタスクへの分解:** 実装エージェントが「設計の意図」を汲み取りつつ迷わずコードを書けるよう、各 Phase を具体的な Issue に落とし込む。
2.  **最終監査と PR 起票:** ロードマップを `docs/architecture/roadmaps/` 等に記録し、PR を起票する。
