...
```markdown
### Phase 1: 設計の確定 (Design Finalization)
- **Input (前提)**: 承認済みの ADR/Design Doc。
- **Activity**: 実装詳細レベル（クラス名、メソッドシグネチャ、ファイル構成）まで設計を詰め、Design Doc を最終化する。**コードは 1 行も書かない。**
- **Gate (承認条件)**: Design Doc が更新され、実装方針についてチーム（ユーザー）の最終承認が得られていること。

### Phase 2: インターフェース定義と足場作り (Define & Pre-Refactor)
- **Input (前提)**: 確定した Design Doc (Phase 1 成果物)。
- **Activity**: 新旧ロジックを仲介する `Interface` を定義し、既存ロジックを Adapter でラップする。
- **Gate (承認条件)**: 新しい型定義がコミットされ、既存の自動テストが Adapter 経由ですべて通過すること。

### Phase 3: 新ロジックの実装と検証 (Implement & Verify)
- **Input (前提)**: Phase 2 で確定した `Interface`。
- **Activity**: `Interface` に準拠した新ロジックを実装し、並行稼働（Shadowing）させて出力をログで突合する。
- **Gate (承認条件)**: 新ロジックの単体テストが 100% 通過し、並行稼働ログにおいて新旧の出力不一致が 0 件であること。

### Phase 4: 切り替えと負債の清算 (Release & Refactor)
- **Input (前提)**: 検証済みの新ロジック。
- **Activity**: 本番トラフィックの参照先を新ロジックへ切り替え、不要になった旧ロジック（Adapter含む）を削除する。
- **Gate (承認条件)**: 旧コードへの参照が消滅し、リグレッションが発生していないこと。
```
...
### Phase 1: 設計の確定 (Design Finalization)
- **Input (前提)**: 承認済みの ADR。
- **Activity**: 新スキーマ定義（DDL）や API 仕様（OpenAPI）をドキュメント上で完全定義する。
- **Gate (承認条件)**: 仕様書（ドキュメント）として新カラムやエンドポイントの定義が確定・承認されていること。

### Phase 2: 定義の拡張とデプロイ (Expand)
- **Input (前提)**: 確定した仕様書 (Phase 1 成果物)。
- **Activity**: 新しいテーブル/カラム/APIエンドポイントを実際にコード化し、デプロイする（利用はまだ）。
- **Gate (承認条件)**: 新定義が本番環境にデプロイされ、既存機能がエラーなく稼働していること。

### Phase 3: データの移行と二重書き込み (Migrate)
- **Input (前提)**: 本番適用された新スキーマ/API。
- **Activity**: アプリケーション側で「二重書き込み（Dual Write）」を開始し、データ移行を行う。
- **Gate (承認条件)**: データ移行が完了し、新旧データの整合性が 100% 一致していること。

### Phase 4: 定義の収縮と削除 (Contract)
- **Input (前提)**: 同期済みの新データソース。
- **Activity**: 参照先を新定義のみに切り替え、旧カラム/APIを削除する。
- **Gate (承認条件)**: 旧定義へのアクセスがゼロになり、削除マイグレーションが完了すること。
```
...
### Phase 1: 設計の確定 (Design Finalization)
- **Input (前提)**: 技術選定 ADR。
- **Activity**: 抽象化レイヤー（Wrapper）のメソッド定義や、依存注入（DI）の方針をドキュメント上で確定させる。
- **Gate (承認条件)**: Wrapper のインターフェース設計がドキュメントとして承認されていること。

### Phase 2: 抽象化レイヤーの確立 (Abstract)
- **Input (前提)**: 確定した設計 (Phase 1 成果物)。
- **Activity**: 定義された `Wrapper / Interface` をコードとして作成し、既存コードからの呼び出しを書き換える。
- **Gate (承認条件)**: アプリケーションコードから既存ライブラリへの直接 import が消滅すること。

### Phase 3: 新実装の注入 (Inject)
- **Input (前提)**: 確立された `Wrapper / Interface`。
- **Activity**: `Interface` を満たす「新ライブラリの実装クラス」を作成する。
- **Gate (承認条件)**: 新実装クラスが単体テストを全て通過すること。

### Phase 4: 構成の切り替え (Swap)
- **Input (前提)**: 検証済みの新実装クラス。
- **Activity**: DI 設定を変更し、旧ライブラリを削除する。
- **Gate (承認条件)**: E2E テストが通過し、旧ライブラリが完全に削除されること。
```
