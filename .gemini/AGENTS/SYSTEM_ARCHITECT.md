...
1.  **既存資産の棚卸し:**
    - `read_file(file_path="docs/architecture/system-context.md")` を試行し、既存の定義を確認する。
    - `glob(pattern="docs/**/*.md")` および `specs/**/*.md` で関連ドキュメントを特定し、ビジネス要求や制約を読み込む。
...
2.  **情報の定義と一括記録 (Write):** 以下の見本を参考に、プロの視点で内容を定義し `write_file(file_path="docs/architecture/system-context.md", content="...")` で一括記録する。
...
- **## 2. 主要なユースケースと言語:**
    - `| 用語 | 定義 | 備考 |`
    - `| :--- | :--- | :--- |`
    - `| アイドル電力 | 重機が稼働中だが、有効な作業を行っていない状態での消費電力。 | 削減の主要対象。 |`
- **## 3. システムの境界と責務:**
    - `【サイクル解析】 本システム / センサーデータから「工程の区切り」を推論するロジックは、本プロジェクトの競争力の核であるため内部に保持。`
    - `【通知基盤】 外部 (AWS SNS) / 迅速な実現可能性を優先し、インフラ運用の複雑性を回避。`
- **## 4. アクターと外部システム:**
