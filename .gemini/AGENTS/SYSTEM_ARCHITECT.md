...
1.  **戦略とフェーズの定義とWBSの展開:**
    ...
    - **WBS の展開:** 定義した各フェーズの直下に、以下の標準構造でタスクテーブル（WBS）を作成し、ロードマップの「## 2. 実装フェーズとWBS」を埋める。
        - **[先頭] Setup Task:** フェーズ用ブランチの作成と環境準備。
        - **[中間] Implementation Tasks:** 具体的な実装・テスト・移行作業（Spike, Pre, Impl, Verify, Clean）。
        - **[末尾] Review Task:** フェーズ完了の承認（Gate チェック）と、次フェーズの計画見直し。
...
| :---: | :---: | :--- | :--- | :---: | :--- |
| T1-1 | Spike | 既存クラスの全依存関係を調査し、リスト化する | `src/legacy/service.py` | - | |
| T1-2 | Pre | 新インターフェース `IService` の定義作成 | `src/new/interface.py` | T1-1 | |
...
| Task ID | Category | タスク内容 | 対象ファイル | 依存先 | Issue案リンク |
| :---: | :---: | :--- | :--- | :---: | :--- |
| T2-1 | Impl | 旧ロジックをラップする `LegacyAdapter` の実装 | `src/new/adapter.py` | T1-2 | |
| T2-2 | Verify | Adapter 経由で既存テストがパスすることを確認 | `tests/unit/test_adapter.py` | T2-1 | |
| T2-3 | Impl | クライアントコードの依存先を `IService` へ変更 | `src/client.py` | T2-2 | |
...
| Task ID | Category | タスク内容 | 対象ファイル | 依存先 | Issue案リンク |
| :---: | :---: | :--- | :--- | :---: | :--- |
| T3-1 | Impl | 新ロジック `NewService` の実装 | `src/new/service.py` | T2-3 | |
| T3-2 | Impl | 新旧比較を行う `ShadowingProxy` の実装 | `src/new/proxy.py` | T3-1 | |
| T3-3 | Verify | 本番ログによる出力一致率の検証 | - | T3-2 | |
...
| Task ID | Category | タスク内容 | 対象ファイル | 依存先 | Issue案リンク |
| :---: | :---: | :--- | :--- | :---: | :--- |
| T4-1 | Impl | DI コンテナの設定変更（NewServiceへの切り替え） | `src/config/container.py` | T3-3 | |
| T4-2 | Verify | リグレッションテストの実行 | - | T4-1 | |
| T4-3 | Clean | 旧コード `legacy/` および `LegacyAdapter` の削除 | `src/legacy/` | T4-2 | |
```

**パターン B: 拡張と収縮 (Expand & Contract) (DB/API変更向け)**
```markdown
### Phase 1: 設計とテスト要件の確定 (Design & Test Criteria)
- **Goal (狙い)**: 破壊的な変更を「安全な拡張」として再定義し、テストによる安全弁を設計する。
- **Input (前提)**: 承認済みの ADR。
- **Deliverables (成果物)**:
    - **検証シナリオ (Validation Scenario)**:
        - **目的**: 新スキーマ/API の正当性を検証する具体的なクエリやリクエストパターンを確定させ、データ破壊リスクを防ぐため。
    - **スキーマ/API 定義ドラフト (Draft Spec)**:
        - **目的**: DDL や OpenAPI 定義をドキュメントレベルで確定させ、実装工程の不確実性を排除するため。
- **Gate (承認条件)**: 検証シナリオと新定義ドラフトが、移行のリスクを十分にカバーしていると合意されること。

### Phase 2: 定義の拡張とデプロイ (Expand)
- **Goal (狙い)**: 既存機能に一切の影響を与えずに「新しい器」を本番環境へ安全に導入する。
- **Input (前提)**: 確定した検証シナリオ。
- **Deliverables (成果物)**:
    - **新定義 (DDL/OpenAPI) とその検証テスト**:
        - **目的**: 既存機能に影響を与えずに、新しいデータ構造を本番環境で利用可能な状態にするため。
- **Gate (承認条件)**: 新定義のデプロイ後、検証テストが本番またはステージングでパスすること。

### Phase 3: データの移行と二重書き込み (Migrate)
- **Goal (狙い)**: 新旧の器を同期させ、リクエストを切り替えても矛盾が起きない状態まで成熟させる。
- **Input (前提)**: 拡張済みの環境。
- **Deliverables (成果物)**:
    - **二重書き込み (Dual Write) 実装とそのテスト**:
        - **目的**: 新規データの整合性を保ちながら、サービスを止めずに移行を進めるため。
    - **データ移行ジョブと整合性検証スクリプト**:
        - **目的**: 過去データを同期し、新旧データが完全に一致していることを機械的に保証するため。
- **Gate (承認条件)**: 移行が完了し、整合性検証テストが 100% パスしていること。

### Phase 4: 定義の収縮と削除 (Contract)
- **Goal (狙い)**: 古い器を廃棄し、不必要な重複と複雑性を解消して「新構造」へ一本化する。
- **Input (前提)**: 同期済みのデータ。
- **Deliverables (成果物)**:
    - **参照切り替えと旧定義削除のマイグレーション**:
        - **目的**: 不要になった旧スキーマ/API を廃止し、システムをシンプルに保つため。
- **Gate (承認条件)**: 削除後のリグレッションテストが全てパスすること。
```

**パターン C: 抽象化と差し替え (ライブラリ置換向け)**
```markdown
### Phase 1: 設計とテスト要件の確定 (Design & Test Criteria)
- **Goal (狙い)**: ツールに依存しない「仕様」をインターフェースとして確立し、交換可能な構造を設計する。
- **Input (前提)**: 技術選定 ADR。
- **Deliverables (成果物)**:
    - **インターフェース契約 (Interface Contract)**:
        - **目的**: 抽象化レイヤー（Wrapper）が満たすべきメソッドと振る舞いを言語化し、実装のすり合わせコストをなくすため。
    - **抽象定義ドラフト (Draft Abstraction)**:
        - **目的**: クラス名や型定義をドキュメント上で確定させ、依存関係の剥がし方を明確にするため。
- **Gate (承認条件)**: インターフェース契約と抽象定義が、既存機能を代替するのに十分であると合意されること。

### Phase 2: 抽象化レイヤーの確立 (Abstract)
- **Goal (狙い)**: 直接依存を剥がして抽象化レイヤーへ誘導し、既存機能の安定性を Interface 経由で再確認する。
- **Input (前提)**: 確定したインターフェース契約。
- **Deliverables (成果物)**:
    - **`Wrapper` 実装と契約テスト**:
        - **目的**: 既存ライブラリをラップし、定義された契約（テスト）を満たす疎結合な状態を作るため。
- **Gate (承認条件)**: 既存ライブラリが Wrapper 経由で動作し、契約テストをパスすること。

### Phase 3: 新実装の注入 (Inject)
- **Goal (狙い)**: 確立した Interface に対して新技術を厳密に実装し、旧技術と同等の品質を証明する。
- **Input (前提)**: 確立された契約テスト。
- **Deliverables (成果物)**:
    - **新ライブラリの実装クラスとそのテスト**:
        - **目的**: 定義された契約テストをパスさせ、ADR で選定した新技術を利用可能にするため。
- **Gate (承認条件)**: 新実装クラスが、Phase 1 で定義されたテストスイートを全てパスすること。

### Phase 4: 構成の切り替え (Swap)
- **Goal (狙い)**: 接続先を入れ替え、古いライブラリを物理的に排除して依存関係をクリーンにする。
- **Input (前提)**: 契約を完全に満たした新実装。
- **Deliverables (成果物)**:
    - **DI 設定の変更と旧ライブラリ削除コミット**:
        - **目的**: 実体を新実装へ切り替え、依存関係をクリーンにするため。
- **Gate (承認条件)**: E2E テストおよび全単体テストがパスすること。
```
...
2.  **Issue 案のファイル出力 (Agent-Ready):** Step 2 で設計した WBS に基づき、以下の3種類の Issue 案を作成する。
    - **A. Phase 開始 Issue (Setup):**
        - 各フェーズの冒頭に配置。作業ブランチ（例: `feature/phase-1-setup`）の作成と、後続タスクへのブランチ共有を指示する。
    ...
    - **C. Phase 完了 Issue (Review & Plan):**
        - 各フェーズの末尾に配置。全タスクの完了確認、成果物の検証（Gate チェック）、および**次フェーズの計画見直し**を指示する。
