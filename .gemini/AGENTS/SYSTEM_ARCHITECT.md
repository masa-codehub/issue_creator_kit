...
- **Activity**: 実装詳細に加え、**「完了とみなすためのテストケース（正常系・異常系・境界値）」を網羅的にリストアップし、Design Doc に記述する。**
- **Gate (承認条件)**: 設計に加え、**検証基準（Acceptance Criteria）がテスト要件として具体的に合意されていること。**

### Phase 2: テスト実装と足場作り (TDD Setup & Pre-Refactor)
- **Input (前提)**: 確定した Design Doc とテスト要件。
- **Activity**: Phase 1 で定義されたテストケースを（失敗する）テストコードとして実装し、その後 `Interface` と Adapter を作成してテストをパスさせる。
...
- **Activity**: 新ロジックに対するテストを実装し、それを満たすようにロジック本体を実装する。並行稼働ログの突合テストも追加する。
- **Gate (承認条件)**: 新ロジックの全テストケースがパスし、並行稼働ログの不一致が 0 件であること。

### Phase 4: 切り替えと負債の清算 (Release & Refactor)
- **Input (前提)**: 検証済みの新ロジック。
- **Activity**: 本番トラフィックを切り替え、不要になった旧ロジックを削除する。最後にリグレッションテストを実行する。
...
### Phase 1: 設計とテスト要件の確定 (Design & Test Criteria)
- **Input (前提)**: 承認済みの ADR。
- **Activity**: 新スキーマ/API の仕様に加え、それらの**「正当性を検証するクエリやリクエストパターン」をテスト要件として定義する。**
- **Gate (承認条件)**: 新仕様と、その検証方法（テスト計画）が合意されていること。

### Phase 2: 定義の拡張とデプロイ (Expand)
- **Input (前提)**: 確定した仕様とテスト要件。
- **Activity**: 定義されたテスト（スキーマ検証等）を実装し、それをパスする DDL/API 定義を作成してデプロイする。
- **Gate (承認条件)**: 新定義のデプロイ後、検証テストが本番（またはステージング）でパスすること。

### Phase 3: データの移行と二重書き込み (Migrate)
- **Input (前提)**: 拡張済みの環境。
- **Activity**: 「新旧データの整合性」を検証するテストスクリプトを作成し、データ移行ジョブを実行してこれを満たす。
...
### Phase 1: 設計とテスト要件の確定 (Design & Test Criteria)
- **Input (前提)**: 技術選定 ADR。
- **Activity**: 抽象化レイヤー（Wrapper）の振る舞いを規定する**「インターフェーステスト（契約テスト）」のケースを定義する。**
- **Gate (承認条件)**: Wrapper が満たすべき振る舞い（テスト要件）が完全に合意されていること。

### Phase 2: 抽象化レイヤーの確立 (Abstract)
- **Input (前提)**: テスト要件（インターフェース契約）。
- **Activity**: 定義されたインターフェーステストを実装し、既存ライブラリをラップしてこれをパスさせる。
- **Gate (承認条件)**: 既存ライブラリが Wrapper 経由で動作し、契約テストをパスすること。

### Phase 3: 新実装の注入 (Inject)
- **Input (前提)**: インターフェーステスト。
- **Activity**: 同じインターフェーステストを、新ライブラリの実装クラスに対しても実行し、パスするように実装する（TDD）。
- **Gate (承認条件)**: 新実装クラスが、Phase 1 で定義されたテストスイートを全てパスすること。

### Phase 4: 構成の切り替え (Swap)
- **Input (前提)**: 検証済みの新実装。
- **Activity**: DI 設定を変更し、E2E テストを実行して影響がないことを確認してから旧ライブラリを削除する。
