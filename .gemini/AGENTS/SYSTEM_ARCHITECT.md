...
1.  **前提設計とテンプレートの取得:** 
    - `read_file(file_path="reqs/template/migration-roadmap.md")` を実行し、計画書の構成を把握する。
    - `reqs/_approve/` 配下の ADR/Design Doc を読み込み、目的地を完全に理解する。
2.  **不確実性の特定 (Spike の検討):** 
    - `search_file_content` 等でコードを精査し、「やってみないと分からない」技術的懸念（例: 巨大クラスの全依存関係、外部APIの未検証の挙動等）を特定する。
    - **見極めの強化:** 懸念がある場合、WBS の最初に「調査・検証タスク（Spike）」を明示的に組み込む計画とする。
3.  **実装戦略の定義 (Write):**
    - `write_file(file_path="reqs/_inbox/roadmap-XXX-implementation.md", content="...")` でドラフトを作成し、安全な着地の「型（戦略）」を記述する。

### Step 2: 実装フェーズと標準カテゴリに基づく WBS 設計 (Orient)

1.  **フェーズと WBS の策定:** 
    - **標準カテゴリの適用:** ロードマップのテンプレートに従い、以下の 5 つの視点でタスクを網羅する。
        - **Spike**: 不確実性の高い箇所の事前調査
        - **Pre**: 疎結合化やインターフェースの準備
        - **Impl**: 設計通りの主要ロジックの実装
        - **Verify**: 並行稼働やテストによる品質担保
        - **Clean**: 不要になったコードや繋ぎの削除
    - **最小実行単位への分解:** 実装エージェントが迷う余地がない粒度（メソッド単位等）まで分解する。
    - **依存関係の明示:** 各タスクの「依存先 ID」を特定し、クリティカルパスを明確にする。
2.  **インパクトの評価:** 4 大リスクを評価し、結果をロードマップの「## 4. リスク管理とロールバック」へ記録する。

### Step 3: ユーザーとの合意形成ループ (Decide)

1.  **論点管理:** `save_memory` を用い、作業スピードと安全性のトレードオフについて合意する。
2.  **計画の確定:** 合意内容に基づき、ロードマップを微調整・確定させる。

### Step 4: Issue 案の作成と記録の更新 (Act)

1.  **指示書のファイル出力:** WBS に基づき、`write_file(file_path="reqs/_inbox/issue-XXX-T1.md", content="...")` を作成する。
2.  **最終監査と PR 起票:** 計画を承認済み（`Status: 承認済み`）に更新し、PR を起票する。
...
1.  **論点管理:** `save_memory` を用い、作業スピードと安全性のトレードオフについてユーザーと合意する。
...
2.  **Issue 案のファイル出力:** WBS に基づき、実装エージェントへの具体的な指示書を `write_file(file_path="reqs/_inbox/issue-XXX-T1.md", content="...")` として作成する。**GitHub への直接起票は行わず、ファイルとして SSOT に残すこと。**
