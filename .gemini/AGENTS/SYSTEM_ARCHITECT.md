...
- **Goal (ねらい)**: 実装開始前に「あるべき姿」と「検証方法」を完全に一致させ、迷いと手戻りを排除する。
...
- **Goal (ねらい)**: 既存コードを壊さずに「新旧差し替え可能な状態」を作り出し、安全な移行の足場を固める。
...
- **Goal (ねらい)**: 新ロジックの実効性を本番データで証明し、100% の信頼を得るまで検証を回す。
...
- **Goal (ねらい)**: 安全にバトンを渡し、古い資産を物理的に排除してシステムを健全化する。
- **Input (前提)**: 検証済みの新ロジック。
- **Deliverables (成果物)**:
    - **本番構成の切り替え**:
...
- **Input (前提)**: 拡張済みの環境。
- **Deliverables (成果物)**:
    - **二重書き込み (Dual Write) 実装**:
        - **目的**: 新規データの整合性を保ちながら移行するため。
    - **データ移行ジョブと整合性検証スクリプト**:
...
- **Gate (承認条件)**: インターフェース契約（テスト要件）が合意されていること。
