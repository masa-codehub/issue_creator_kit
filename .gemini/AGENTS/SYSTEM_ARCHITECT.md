...
```markdown
### Phase 1: インターフェース定義と足場作り (Design & Pre-Refactor)
- **Input (前提)**: 承認済みの Design Doc と、現状の依存関係調査結果。
- **Activity**: 新旧ロジックを仲介する `Interface` を定義し、既存ロジックを Adapter でラップする。OpenAPI や型定義ドキュメントもここで確定させる。
- **Gate (承認条件)**: 新しい型定義がコミットされ、**既存の自動テストが Adapter 経由ですべて通過すること**。

### Phase 2: 新ロジックの実装と検証 (Implement & Verify)
- **Input (前提)**: Phase 1 で確定した `Interface` とテスト環境。
- **Activity**: `Interface` に準拠した新ロジックを実装し、旧ロジックと並行稼働（Shadowing）させて出力をログで突合する。
- **Gate (承認条件)**: 新ロジックの単体テストが 100% 通過し、**並行稼働ログにおいて新旧の出力不一致が 0 件であること**。

### Phase 3: 切り替えと負債の清算 (Release & Refactor)
- **Input (前提)**: 検証済みの新ロジックと、突合完了ログ。
- **Activity**: 本番トラフィックの参照先を新ロジックへ切り替え、不要になった旧ロジック（Adapter含む）を削除する。
- **Gate (承認条件)**: 旧コードへの参照（import）が物理的に消滅し、**リグレッションが発生していないこと**。
```

**パターン B: 拡張と収縮 (Expand & Contract) (DB/API変更向け)**
```markdown
### Phase 1: 定義の拡張とデプロイ (Expand)
- **Input (前提)**: 承認済みの ADR (新スキーマ/API設計)。
- **Activity**: 新しいテーブル/カラム/APIエンドポイントを追加する（この時点ではデータは流さない、または Nullable）。
- **Gate (承認条件)**: 新しい定義が本番環境にデプロイされ、**既存アプリケーションがエラーなく稼働し続けていること**。

### Phase 2: データの移行と二重書き込み (Migrate)
- **Input (前提)**: 本番適用された新スキーマ/API。
- **Activity**: アプリケーション側で「新旧両方への書き込み（Dual Write）」を開始し、バックグラウンドで過去データの移行を行う。
- **Gate (承認条件)**: 全データの移行が完了し、**新旧データの整合性が 100% 一致していること**。

### Phase 3: 定義の収縮と削除 (Contract)
- **Input (前提)**: 同期済みの新データソース。
- **Activity**: アプリケーションの参照先を新定義のみに切り替え、旧カラム/APIを削除する。
- **Gate (承認条件)**: 旧定義へのアクセスログがゼロになり、**削除マイグレーションが正常に完了すること**。
```

**パターン C: 抽象化と差し替え (ライブラリ置換向け)**
```markdown
### Phase 1: 抽象化レイヤーの確立 (Abstract)
- **Input (前提)**: 技術選定 ADR と、既存ライブラリの使用箇所リスト。
- **Activity**: 依存ライブラリを隠蔽する `Wrapper / Interface` を作成し、既存コードからの呼び出しをすべてここ経由に書き換える。
- **Gate (承認条件)**: アプリケーションコードから既存ライブラリへの直接 import が消滅し、**Wrapper 経由での動作が保証されること**。

### Phase 2: 新実装の注入 (Inject)
- **Input (前提)**: 確立された `Wrapper / Interface`。
- **Activity**: `Interface` を満たす「新ライブラリの実装クラス」を作成し、単体テストを記述する。
- **Gate (承認条件)**: 新実装クラスが `Interface` の契約（仕様）を完全に満たし、**単体テストを全て通過すること**。

### Phase 3: 構成の切り替え (Swap)
- **Input (前提)**: 検証済みの新実装クラス。
- **Activity**: DI コンテナの設定変更により実体を新実装へ切り替え、旧ライブラリをプロジェクトから削除する。
- **Gate (承認条件)**: `package.json` 等から旧ライブラリが削除され、**E2E テストが通過すること**。
```
