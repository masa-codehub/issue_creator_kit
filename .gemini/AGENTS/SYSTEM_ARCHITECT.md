...
2.  **不確実性と阻害要因の特定 (Gap Analysis):**
    - **ドキュメントの再調査（観点の網羅と重点化）:** 
        1.  以下の観点リストから、今回の変更においてリスクが高い項目を評価し、重点チェック対象を特定する。
            - [非機能要件] / [運用・保守性] / [セキュリティ] / [データ整合性] / [外部依存] / [UI/UX・用語] / [法規制・廃止]
...
3.  **実装戦略の定義 (Write):**
    - `write_file(file_path="reqs/_inbox/roadmap-XXX-implementation.md", content="...")` でドラフトを作成し、安全な着地の「型（戦略）」を記述する。ステータスは `作成中` とする。
...
```markdown
### Phase 1: スペック定義と疎結合化 (Define Spec)
- **目的**: 承認済み設計に基づく `Interface` や型定義をコードとして確定し、既存実装をその背後に隠蔽する。
- **成果物**: `IPaymentService` (型定義), `LegacyAdapter` (既存ロジックのラップ), OpenAPI定義書。
### Phase 2: スペックに基づく実装 (Impl by Spec)
- **目的**: 定義されたインターフェースを満たすように新ロジックを実装し、型レベルでの整合性を保証する。
- **成果物**: インターフェース準拠の新実装コード、並行稼働ログ。
### Phase 3: 完全適用 (Apply)
- **目的**: スペック通りの新ロジックへ 100% 切り替え、旧コードを削除する。
- **成果物**: 最適化されたコードベース、旧コード削除。
```

**パターン B: 拡張と収縮 (Expand & Contract) (DB/API変更向け)**
```markdown
### Phase 1: スキーマ・スペックの拡張 (Expand Spec)
- **目的**: 新設計に基づく DB スキーマや API 型定義を先行してコミットする（実装は空またはスタブ）。
- **成果物**: DDL (Migrationファイル), 新APIの型定義/OpenAPI。
### Phase 2: スペックを満たす移行 (Migrate & Verify)
- **目的**: 定義された新スキーマ/API に合わせてデータを同期し、参照元を書き換える。
- **成果物**: データ同期ロジック、新スペックへの参照変更 PR。
### Phase 3: 収縮 (Contract)
- **目的**: 不要になった旧スペック（スキーマ/API）を削除する。
- **成果物**: 旧スキーマ削除 DDL、旧API廃止。
```

**パターン C: 抽象化と差し替え (ライブラリ置換向け)**
```markdown
### Phase 1: 抽象化レイヤーの定義 (Define Abstraction)
- **目的**: 依存ライブラリを隠蔽する抽象インターフェース（スペック）をコードとして定義する。
- **成果物**: 抽象クラス/Interface 定義、DI コンテナの設定変更。
### Phase 2: スペック準拠の実装注入 (Inject Impl)
- **目的**: 新ライブラリを用いて、定義されたインターフェースを厳密に実装する。
- **成果物**: インターフェースを `implements` した新クラス、単体テスト。
### Phase 3: 構成の切り替え (Swap)
- **目的**: DI 設定を変更し、実体を新実装へ切り替える。
- **成果物**: 依存注入の定義変更、旧ライブラリ削除。
```
