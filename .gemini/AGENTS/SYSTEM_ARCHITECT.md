...
- **Activity**:
    - **`roadmap-XXX-implementation.md` (検証基準セクション)**:
        - **目的**: 実装着手前に正常系・異常系・境界値を含む「成功の定義」をテストレベルで言語化し、実装時の不確実性を排除するため。Design Doc は更新せず、この計画書内でテスト仕様を確定させる。
- **Gate (承認条件)**: ロードマップに記述された検証基準（Acceptance Criteria）が、設計の意図を完全に網羅していると合意されること。

### Phase 2: テスト実装と足場作り (TDD Setup & Pre-Refactor)
- **Input (前提)**: 確定したロードマップの検証基準。
- **Activity**:
    - **失敗するテストコード (Red Test)**:
        - **目的**: Phase 1 の要件をコードとして表現し、実装のゴールを機械的に検証可能にするため。
    - **`Interface` と `Adapter`**:
        - **目的**: テストをパスさせ、新旧ロジックを切り替え可能な疎結合状態にするため。
- **Gate (承認条件)**: 定義されたインターフェースのテストが全てパスし、既存コードが Adapter 経由で動作すること。

### Phase 3: 新ロジックの実装と検証 (TDD Implementation)
- **Input (前提)**: Phase 2 で確立されたテスト環境。
- **Activity**:
    - **新ロジックの実装コード**:
        - **目的**: ロードマップの検証基準を満たし、正しいビジネスロジックを実現するため。
    - **並行稼働ログ (Shadowing Log)**:
        - **目的**: 本番トラフィックを用いた比較検証により、潜在的なバグやエッジケースの不一致を検出するため。
- **Gate (承認条件)**: 新ロジックの全テストケースがパスし、並行稼働ログの不一致が 0 件であること。

### Phase 4: 切り替えと負債の清算 (Release & Refactor)
- **Input (前提)**: 全テストをパスした新ロジック。
- **Activity**:
    - **本番構成の切り替え**:
...
- **Activity**:
    - **`roadmap-XXX-implementation.md` (検証基準セクション)**:
        - **目的**: 新スキーマ/API の正当性を検証するクエリやリクエストパターンを定義し、移行時のデータ破壊リスクを防ぐため。
- **Gate (承認条件)**: ロードマップ上の検証基準（テスト計画）が、移行のリスクを十分にカバーしていると合意されること。

### Phase 2: 定義の拡張とデプロイ (Expand)
- **Input (前提)**: 確定したロードマップの検証基準。
- **Activity**:
    - **スキーマ検証テスト**:
        - **目的**: 新しい定義が正しくデプロイされ、アプリケーションから利用可能であることを保証するため。
    - **新定義 (DDL/OpenAPI) のデプロイ**:
        - **目的**: 既存機能に影響を与えずに、新しいデータ構造を利用可能な状態にするため。
...
- **Activity**:
    - **整合性検証スクリプト**:
        - **目的**: 新旧データが完全に一致していることを機械的に保証するため。
    - **二重書き込み (Dual Write) と移行ジョブ**:
        - **目的**: サービスを止めずに、過去データと新規データを新スキーマへ同期するため。
- **Gate (承認条件)**: 整合性検証テストが 100% パスしていること。
...
- **Activity**:
    - **`roadmap-XXX-implementation.md` (検証基準セクション)**:
        - **目的**: 抽象化レイヤー（Wrapper）が満たすべき「契約（Interface Test）」を言語化し、実装のすり合わせコストをなくすため。
- **Gate (承認条件)**: ロードマップに記述されたインターフェース契約（テスト要件）が合意されていること。

### Phase 2: 抽象化レイヤーの確立 (Abstract)
- **Input (前提)**: ロードマップの検証基準（契約）。
- **Activity**:
    - **契約テストコード (Contract Test)**:
        - **目的**: ライブラリの実装に依存せず、インターフェースの振る舞いを保証するため。
    - **Wrapper 実装と既存コードの書き換え**:
        - **目的**: 依存ライブラリを隠蔽し、テスト可能な状態にするため。
