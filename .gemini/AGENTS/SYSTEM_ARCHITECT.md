...
- **成果物**: `roadmap-XXX-implementation.md` (## 5. 検証基準 セクション)
- **目的**: 実装着手前に、正常系・異常系・境界値を含む「成功の定義」をテストレベルで言語化し、実装工程での不確実性を排除する。**Design Doc は更新せず、計画書内でテスト仕様を確定させる。**
...
- **Activity**: Phase 1 で定義されたテスト要件を（失敗する）テストコードとしてまず実装する。その後 `Interface` と Adapter を作成し、そのテストをパスさせる。
...
- **Activity**: ロードマップの検証基準に基づき、新ロジックに対するテストを先に実装し、それを満たすようにロジック本体を実装する。
- **Gate (承認条件)**: 新ロジックの全テストケースがパスし、並行稼働ログの不一致が 0 件であること。

### Phase 4: 切り替えと負債の清算 (Release & Refactor)
- **Input (前提)**: 全テストをパスした新ロジック。
- **Activity**: 本番トラフィックを切り替え、不要になった旧ロジックを削除する。最後に全テストスイートを実行する。
...
- **Input (前提)**: 承認済みの ADR。
- **成果物**: `roadmap-XXX-implementation.md` (## 5. 検証基準 セクション)
- **目的**: 新スキーマ/API の正当性を検証するための「具体的なクエリやリクエストパターン」を、実装着手前にテスト要件として確定させる。
- **Gate (承認条件)**: ロードマップ上の検証基準（テスト計画）が、移行のリスクを十分にカバーしていると合意されること。

### Phase 2: 定義の拡張とデプロイ (Expand)
- **Input (前提)**: 確定したロードマップの検証基準。
- **Activity**: スキーマ検証テストを実装し、それをパスする DDL/API 定義を作成してデプロイする。
- **Gate (承認条件)**: 新定義のデプロイ後、検証テストが本番またはステージングでパスすること。

### Phase 3: データの移行と二重書き込み (Migrate)
- **Input (前提)**: 拡張済みの環境。
- **Activity**: 「新旧データの整合性」を検証するテストスクリプトを先行実装し、それをパスするように移行ジョブを調整・実行する。
- **Gate (承認条件)**: 整合性検証テストが 100% パスしていること。

### Phase 4: 定義の収縮と削除 (Contract)
- **Input (前提)**: 同期済みのデータ。
- **Activity**: 参照切り替えを行い、旧定義削除のマイグレーションを実行する。
