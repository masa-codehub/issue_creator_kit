---
name: adr-decision-support
description: Skill for facilitating architectural decisions through hypothesis verification. Identifies information gaps based on architectural values and factual clarity, asks a single high-impact question, and analyzes the response to converge on a decision.
---

# 意思決定支援 (ADR Decision Support)

ADR作成に必要な情報の「欠損（Gap）」を特定し、最も効果的な「たった一つの質問」を通じて仮説を検証・収束させるスキル。
...
## 前提 (Prerequisites)
- アーキテクチャ仮説（案A, B, C）が提示されていること。

## 手順 (Procedure)

### 1. 欠損情報のリストアップ (Gap Analysis)
- **Action:**
  - 現在の手持ち情報と、ADRとして完成させるために必要な情報、およびSYSTEM_ARCHITECTの価値観を照合する。
...
### 3. ADR反映事項の特定 (Define Reflection Points)
- **Action:**
  - ユーザーの回答を分析し、仮説検証の結果として **「ADRに具体的に何を記述・修正すべきか」** を特定して提案する。
  - 判断や分岐は行わず、あくまで「回答に基づいた記述方針」を出力する。

- **Example Output:**
  - 「回答を承知しました。ADRには『初期コストより速度を優先するため案Aを採用』とし、リスク欄に『半年後のリファクタリングを前提とする』旨を追記します。」

## 完了条件 (Definition of Done)
- ユーザーの回答に基づき、ADRに記述すべき具体的な内容（反映事項）が特定されていること。