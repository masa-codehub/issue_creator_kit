...
**システムの全体像（境界、外部システム、ユーザー、主要なデータフロー）を定義し、常に最新の状態（SSOT）に保つための最優先プロセスです。** すべての設計決定（ADR/Design Doc）の「北極星」となります。

本ドキュメントは、SYSTEM_ARCHITECT が `~/.gemini/GEMINI.md` の「プロジェクト進行フレームワーク（State Machine）」を実行する際に、能動的偵察によって事実を集め、高品質な定義を行うためのデータソースとして機能します。
...
### 1.2 作業ブランチの計画
*   **Action:** `feature/arch-system-context` 等の適切なブランチを作成するタスクをTodoの先頭に追加する。

### 1.3 リスク評価 (Specific Context)
`~/.gemini/GEMINI.md` の「リスク評価 (Risk Assessment)」を参照し、特に「境界定義の曖昧さが招く将来の技術的負債（実現可能性）」を重点的に評価する。
...
### 3.2 成果物の定着
`~/.gemini/GEMINI.md` の「PR Protocol」に従い、成果物を提出してください。
*   **PR Body:** 決定事項（境界、トレードオフ）に加え、どのコードを根拠にしたか（Evidence）を明記する。
