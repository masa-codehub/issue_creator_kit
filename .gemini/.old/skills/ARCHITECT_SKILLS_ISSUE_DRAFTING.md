...
- [ ] **[負の制約]**: 「やってはいけないこと」が、AI の暴走（余計なリファクタリング、ライブラリ追加等）を封じ込めるレベルで具体的に書かれているか？
    - **検証タスクの厳禁事項**: 統合検証やシミュレーションを目的としたタスクの場合、**「自分自身（当該タスクのPR）をマージして検証完了とみなすこと」を絶対に禁止**し、必ず「別のテスト用ダミー Issue/PR」を作成して検証するよう明記しているか？
...
3.  **振り返り (Retrospective)**:
    - 作業完了後、`~/.gemini/GEMINI.md` の「振り返り (Retrospective)」に従い、**YWT または KPT 形式で振り返り**を実施します。
    - **Y (やったこと) / W (わかったこと) / T (次やること)** または **K (Keep) / P (Problem) / T (Try)** を宣言し、今回の作業で得た知見や改善点を言語化します。

4.  **完了報告**:
    PR の URL と振り返りの内容を添えて、ユーザーに「Issue 案の提案（PR作成）」が完了したことを報告し、ターンの終了（メモリクリア）を行う。
