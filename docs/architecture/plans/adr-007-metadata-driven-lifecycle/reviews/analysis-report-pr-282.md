...
- **GitHub**: `create_issue_from_document(doc: Document)` を追加。`Document` オブジェクトを直接受け取り、内部でマッピング規約（Title: `doc.metadata["id"] + doc.title`, Body: `doc.content + footer`）を適用する。
...
2. `GitHubAdapter.sync_issue(doc: Document) -> int`: 起票または更新を行い、`issue_id` を返す。
