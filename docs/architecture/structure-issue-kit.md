        ...
        subgraph "Application Core"
            ...
            subgraph "Layer: Domain (Core / Scanner Foundation)"
                ...
                subgraph "Scanner Foundation"
                    SVC_SCAN[ScannerUseCase]
                    Visualizer[Visualizer]

                    SVC_SCAN -.-> FSS[FileSystemScanner]
                    SVC_SCAN -.-> Parser[DocumentParser]
...
- **Role (Domain-Centric):** ユーザー（GitHub Actions）からの実行 指示（`visualize` 等）を受け取り、必要なAdapterを選択して ScannerUseCase を起動する。
- **Layer (Clean Arch):** Interface (Controller)
- **Dependencies:**
  - **Downstream:** ScannerUseCase, Visualizer, Infrastructure (for DI)
...
- **Reference:** [Scanner Foundation Structure (ADR-008)](structure-scanner.md)
- **Role (Domain-Centric):** 物理ファイルシステムの走査、Markdown 解析、依存関係グラフ（DAG）の構築およびグラフの可視化を統括する。
- **Components:**
  - **ScannerUseCase**: 統合Facade。ファイルシステムスキャンとパースを調整する。
  - **FileSystemScanner**: `reqs/` を走査し、未処理ファイルを抽 出する。
  - **DocumentParser**: Markdown のメタデータをパースし、Domain Entity へ変換する。
...
- **Capabilities (Extended):**
  - **GitHubAdapter**: Label 検索 (`search_issues_by_label`), Label の存在保証 (`ensure_label`), および非表示コメントによるメタデータ埋め込み (`create_issue_with_metadata`) をサポートし、自動化の冪等性を担保する。
- **Layer (Clean Arch):** Infrastructure
- **Dependencies:**
  - **Upstream:** CLI (Instantiation), UseCases (L1Automation etc.)
