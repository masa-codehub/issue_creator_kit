...
3. メタデータの `issue` フィールド（`#123` / `123` いずれの形式も許容）について、抽出した番号と一致するファイルを特定する。
...
2. **ファイル移動**:
    ...
    - 移動先: 移動元パスの `drafts` ディレクトリ名を `archive` に置換したパス (例: `reqs/tasks/archive/phase-2`)
...
| 移動先のパスが既に存在する | 処理をスキップし、不整合（既存ファイルの破壊）を防止する。 |
| 新規ブランチ名が既に存在する | 処理をスキップし、重複した PR の作成を防止する（冪等性の担保）。 |
