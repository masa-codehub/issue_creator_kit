...
- **Proposed Action:**
  ...
  - **Mermaid Rules:** - Arrow direction: `From --> To` (Depends on). - Empty graph: Always return `graph TD`. - Deterministic order: Nodes by ID asc, Edges by (From, To) asc. - Title resolution: `metadata.extra["title"]` -> H1 -> ID. - Escaping: `"` -> `"`, `[` -> `(`, `]` -> `)`, `
` -> `<br/>`.
