...
| **S1-5**  | `components/infra_adapters.md` | `GitAdapter.get_added_files`, `GitHubAdapter.find_or_create_issue` 等のシグネチャ。 |
| **S1-6**  | `api/cli_commands.md`          | `process-diff`, `process-merge` コマンドの引数と終了コード、例外の外部露出。        |
