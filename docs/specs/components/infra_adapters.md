...
- **マッピングルール**:
  - **Title**: `doc.metadata["id"]` + `": "` + `doc.title` (例: `007-T1: Implement something`)
  - **Body**: `doc.content` + 追記情報（メタデータ一覧を Markdown テーブル形式で末尾に付与）
  - **Labels**: `doc.metadata["phase"]`, `doc.metadata["type"]` などを付与。
- **ロジック**:
  - すでに `doc.metadata["issue_id"]` が存在する場合は、その Issue を更新する。
...
### 5.7. `find_file_by_id(task_id: str, search_dirs: list[str]) -> str`
...
- **戻り値**: 見つかったファイルのパス（`str`）。見つからない場合は `None` ではなく `FileSystemError` を送出する。
- **例外**: `FileSystemError`
...
| ステップ | アクション | 失敗時の振る舞い |
| :--- | :--- | :--- |
| 1. GitHub Sync | `GitHubAdapter.sync_issue(doc)` を実行し、`issue_id` を取得。 | 処理中断。ファイルは元の位置に残り、リトライ可能。 |
| 2. Metadata Injection | `FileSystemAdapter.update_metadata(path, {"issue_id": id, "status": "Issued"})` を実行。 | 処理中断。ファイルは元の位置に残り、メタデータは（不完全だが）次回再試行で上書きされる。 |
| 3. Physical Move | `FileSystemAdapter.safe_move_file(path, "_archive/")` を実行。 | `FileSystemError` 送出。ファイルは不整合状態（Issued だが元の位置）になるが、`find_file_by_id` で修復可能。 |
