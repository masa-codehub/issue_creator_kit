# Use Case Orchestrator Specification (ADR-017)

## 1. Overview

- **Responsibility**: ドラフトの走査、検証、起票、および事後処理（削除）の一連のワークフロー（Relay/Activation）を統括する。システム全体の状態を同期させ、不整合のない起票プロセスを保証する。
- **Collaborators**: `ScannerService`, `Validator`, `GraphBuilder`, `L1SyncService`, `IssueRenderer`, `Interfaces` (GitHub/File Adapters).
- **State Machine Proof**: [state-machine.json](./state-machine.json)
- **Mathematical Assertions**: [state-machine-assertions.test.ts](./state-machine-assertions.test.ts)
