# Metadata-Driven Architecture Structure
...
C4Container
    ...
    Container(ick, "issue-kit CLI", "src/issue_creator_kit/", "Metadata parser and lifecycle engine")
...
- **Dependencies:**
  ...
  - **Upstream:** `Architect (Human)`, `issue-kit CLI`
...
- **Data Reliability:** `issue-kit CLI` による移動（Atomic Move）により 、重複起票を防止。

- **Trade-off:** 物理ファイルをアーカイブとして残すことで、GitHub がダウンしても設計意図とタスクの履歴を Git 上で追跡可能にしている 。

### issue-kit CLI (Lifecycle Engine)
...
参照: [Physical State Lifecycle (ADR-008)](state-lifecycle.md)
...
| Rule                      | Description                                                                                                        | Implementation Target          |
| :------------------------ | :----------------------------------------------------------------------------------------------------------------- | :----------------------------- |
| **Strict Dependency**     | 全ての `depends_on` 対象が `Issued` または `Completed` 状態になるまで、自身の `status` は `Ready` に移行できない。 | `issue-kit CLI / Sync Logic`   |
| **Atomic Issue Creation** | GitHub Issue が正常に作成されない限 り、物理ファイルは `_archive/` へ移動してはならない。                          | `issue-kit CLI / Create Logic` |
| **Hierarchy Priority**    | L3 タスクは、親となる L1 (ADR) および L2 (統合タスク) が `Issued` 以上であることを要する。                         | `issue-kit CLI / Validation`   |
