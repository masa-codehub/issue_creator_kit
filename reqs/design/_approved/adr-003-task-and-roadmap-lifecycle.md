...
1. drafts/からarchive/への*.mdの移動（プルリク#1を手動作成）
2. プルリク#1 をmainにマージ
3. Issueを自動起票（移動した*.mdのみ）
4. Issueを全て実装しphaseのブランチへマージ
5. 最終Issueを完了（phaseのブランチをmainにマージ）
6. 次のフェーズの*.mdをdrafts/からarchive/へ移動（プルリク#Nを自動起票、どうやって最終Issueか判断するかは未定）
7. プルリク#7 をmainにマージ
8. Issueを自動起票、以下そのadr/design-docのIssueが全て処理されるまで続く
