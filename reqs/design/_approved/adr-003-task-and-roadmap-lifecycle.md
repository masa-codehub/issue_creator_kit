...
この設計の本質は、「起票（開始）」と「完了（次への連鎖）」の明確な分離である。

#### 1. タスクの発生（起票フェーズ）
まず、タスクは **drafts** という準備室で練られる。準備が整うと、人間（または AI エージェント）が手動で PR（プルリクエスト #1）を作成し、ファイルを **archive**（実行フロア）へ移動させる。
この PR がマージされると、システムは「新しいタスクが投入された」と認識し、Issue を自動起票する。
この時点では、タスクは「始まったばかり」である。したがって、次のフェーズのことを考えるのは早すぎる。システムは「起票」だけを行い、静かに待機する。

#### 2. タスクの遂行（実装フェーズ）
人間（または AI エージェント）は起票された Issue に基づき、実装作業を行う。タスクごとにブランチを切り、フェーズブランチへ集約していく。
この間、システムは関与しない。人間が主導権を握る。

#### 3. タスクの完了と連鎖（完了フェーズ）
フェーズ内の全てのタスクが実装され、最終 Issue（例: 完了監査やマージ作業）の PR が **main** にマージされた時、これが「フェーズ完了」の合図となる。
システムはこの合図（完了イベント）を検知して初めて、「次のフェーズの準備」に着手する。
具体的には、次フェーズの **drafts** ファイルを **archive** へ移動させるための新しい PR（プルリクエスト #N）を自動作成する。

#### 4. 循環（サイクル）
人間がその自動作成された PR（#N）を確認し、マージすると、ステップ 1 に戻る。
次フェーズのファイルが **archive** に入り、また新たな Issue が起票され、次の開発サイクルが始まる。
