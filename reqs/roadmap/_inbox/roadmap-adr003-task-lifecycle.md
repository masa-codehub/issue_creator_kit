...
| T1-5 | Pre | 制御用メタデータスキーマ確定（`next_phase_path`）とテンプレート最終更新 | `issue-draft.md` | T1-3 |
...
- **リスク**: マージ差分の検知漏れ（force push 等による）。
- **対策**: 差分検知に加え、定期的な「未採番ファイルスキャン」を併用するか検討する（Phase 1 で判断）。
