...
VALID_ROLES = {"arch", "spec", "implement"}
VALID_PHASES = {"arch", "spec", "implement"}
LEGACY_PHASES = {
    "domain",
    "infrastructure",
    "usecase",
...
def validate_role_compatibility(v: str | None) -> str | None:
    """Validate role against new standards (with legacy support)."""
    if v is None:
        return None
    if v in VALID_ROLES:
        return v
    if v in LEGACY_ROLES:
        return v
    raise ValueError(
        f"Invalid role: {v}. Expected one of {VALID_ROLES} or legacy {LEGACY_ROLES}"
    )


def validate_phase_compatibility(v: str | None) -> str | None:
    """Validate phase against new standards (with legacy support)."""
    if v is None:
        return None
