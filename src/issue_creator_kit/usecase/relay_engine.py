...
class RelayEngine:
    ...
    def _handle_phase_transition(
        ...
        """
        # Map legacy roles/phases to current standard (implement)
        role_to_phase_map = {
            "tdd": "implement",
            "impl": "implement",
        }
        role = source_task.role
        target_phase = role_to_phase_map.get(role, role) if role else None
        ...
        if current_phase:
            if current_phase != target_phase:
                logger.info(
                    ...
                    f"but completed task role is '{source_task.role}' (mapped to '{target_phase}'). Skipping."
                )
                return
            base_phase = current_phase
        else:
            # Inference case (Spec 4.3): Treat task role's mapped phase as current phase
            if target_phase not in self.PHASE_ORDER:
                logger.info(
                    f"Cannot infer phase: Task role '{source_task.role}' (mapped to '{target_phase}') is not in PHASE_ORDER. Skipping."
                )
                return
            base_phase = target_phase
            logger.info(
